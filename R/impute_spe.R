...
impute_spe <- function(spe,
  ...
  if (method %in% c("zero", "mean", "median", "median_half")) {
    ...
    for (i in setdiff(seq_along(1:nrow(dat)), fix_prots)) { ## for all of the values we WANT to fix
    ...
    for (s in spatUnits) {
      ...
      if (method == "group_mean") {
        ...
        for (i in setdiff(seq_along(1:nrow(imputed_data)), fix_prots)) { ## for all of the values we WANT to fix
        ...
        for (f in fix_prots) {
          ...
          } # Keep those missing values as NA since there is not enough dat to impute reliably.
        }
      }
