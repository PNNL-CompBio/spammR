...
spatial_heatmap <- function(spe,
  ...
  if (is.null(spot_size_name)) {
    ...
    spot_width <- SummarizedExperiment::colData(spe)[, spot_size_name[1], drop = TRUE]
    spot_height <- SummarizedExperiment::colData(spe)[, spot_size_name[2], drop = TRUE]
  }
  ...
  if (is.na(feature_type)) { # default is whatever is used for rownames of f
    ...
    rowNum_toplot <- which(SummarizedExperiment::rowData(spe)[, feature_type, drop = TRUE] %in% feature)
  }
