...
volcano_plot <- function(diffex.spe,
                         ...
                         thresh = 0.05,
                         sigLabel_colname) {
    diffEx_df <- data.frame(SummarizedExperiment::rowData(diffex.spe), 
                            check.names = FALSE)
    if (missing(pval_colname)) {
      pval_colname <- colnames(diffEx_df)[grep("adj.P.Val", 
                                         colnames(diffEx_df), fixed = TRUE)[1]]
    }
    if (missing(logFC_colname)) {
      logFC_colname <- colnames(diffEx_df)[grep("logFC", 
                                          colnames(diffEx_df), fixed = TRUE)[1]]
    }
    pval_type_lbl <- ""
    if (pval_corrected) {
    ...
    if (length(sig_indices > 0)) {
      p_title <- paste(p_title, "\nAnnotated points show ", 
                       sigLabel_colname, " names", sep = "")
    }
    text_xcoord <- (min(xdat, na.rm = TRUE) + max(xdat, na.rm = TRUE)) / 2 
    # midpoint of x-axis
    p2 <- ggplot2::ggplot(diffEx_df, 
      ...
      ggplot2::annotate("text", 
                        x = text_xcoord + 0.2 * text_xcoord, 
                        y = (-log10(thresh) + 0.05), 
                        label = paste(pval_type_lbl, " < ", 
                                      thresh, sep = ""), 
                        color = "blue") +
