...
#' @description calc_missingness() calculates the fraction of samples that are missing data for each feature, and fraction of each feature that are missing for each sample. 
#' @param spe SpatialExperiment object
#' @param group_name Name of group to use as denominator
#' @return SpatialExperiment object with missingFeature column added to colData and missingSample added to rowData
#' @export
#' @examples
#' library(spammR)
#' data(smallPancData)
...
calc_missingness <- function(spe, group_name=NULL){
  ...
  if(is.null(group_name)){
    ...
    for(g in groups){
      ...
      msamp <- apply(assay(spe)[,samps],1,function(x) length(which(is.na(x)))/length(x))
