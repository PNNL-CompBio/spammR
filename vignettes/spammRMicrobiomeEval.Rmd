...
sd <- download.file('https://figshare.com/ndownloader/files/55158824',
                    method='libcurl',dest='sdata.rda', quiet=TRUE)
load("sdata.rda")
...
maprda = download.file('https://figshare.com/ndownloader/files/55158815', method = 'libcurl',
...
```
## CLEANING UP OF DATA
@Yannick can we remove these? i'm not sure why they are here if not being evaluted
...
Why only use a subset of the data?

```{r}
data_meta_testing <- subset(data_meta, 
                            Sample_ID %in% c('ANZA_TOP', 'ANZA_BTM', 'CFS1_TOP', 'CFS1_BTM'))
...
```{r}
kk <- download.file('https://figshare.com/ndownloader/files/55158818',method='libcurl',destfile='kegg.rda',quiet=TRUE)
load('kegg.rda')
