...
```{r lipid enrichment, warning=FALSE, message=FALSE}

download.file("https://api.figshare.com/v2/file/download/60018356", 
                    destfile = "lipids.tsv.gz", quiet = TRUE)

class <- readr::read_delim("lipids.tsv.gz", delim = '\t')
...
Only two lipids show up in our graph here, but we can plot the expression of 
those in the heatmap as well to observe their expression in the ROI where there 
is protein but also across the image
...
Hk1 also correlated with a second lipid, though the two were not 
correlated with each other, indicating complementary activity elsewhere.
...
p1 <- spatial_heatmap(reduced, assay_name = 'lipids',
                ...
                plot_log = TRUE, 
                metric_display = 'Lipid expression')
p2 <- spatial_heatmap(reduced, assay_name = 'lipids',
                feature = li$name[2],
                sample_id = 'rat_brain',
                image_id = 'rat_brain', 
                plot_log = TRUE, 
