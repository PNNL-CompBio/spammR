...
library(leapR) ##this is on bioconductor
...
Now we download the data we need.
...
should you choose to load teo GEOJson using the `geojsonR` package. 

```{r format coordinates, warning=FALSE, message=FALSE, eval = FALSE}
#remove this once we can include it in spammR
 ...
 #y-coordinates are from top, so need to udpate
  ...
  write.csv(as.data.frame(coords),file='../inst/extdata/bcoords.csv')
...
The result is a `SpatialExperiment`. This takes too long to run as a vignette
so we show the code and then load the file separately.

```{r metaspace import and plotting, warning=FALSE, message=FALSE, eval = FALSE}
...
```{r udpate ms object, error = FALSE}

mspe.url <- 'https://api.figshare.com/v2/file/download/60993349'
mcache <- bfcadd(bfc, "mspe", fpath = mspe.url)
...
Lastly we add the image file to the data downloaded by metaspace and visualize a
lipid of interest. 
```{r add image}
...
to the same regions as the proteomics.  Here is an example of the same ion
we saw before. 
...
The genes and proteins of the brain have been well studied, and proteins such
as glial marker Gfqp are known to play a significant role in the central nervous 
system and can be used as markers for disease. 
We can now search for lipids that are correlated with myelin
spatially using our network tool.

```{r gfap cors, warning = FALSE, message = FALSE}
...
Now we can now double check the lipid measurements of these values. As a 
refresher here is myelin.
...
#ggsave('gfap_brain.pdf')
```

And here we can see how the correlated lipids are expressed throughout.
...
cowplot::plot_grid(c1, c2, ncol = 2)
#ggsave('correlatedLipids.pdf', width = 10)
```
          
This approach, if used on larger samples, can be used to identify  lipid 
biomarkers based on known protein function. 
          
### Ascribing function to unknown lipids

We can also use the correlation analysis to assess the biological function
...
We'll focus on phosphatidylserine "PS(28:6(10Z,13Z,16Z,19Z,22Z,25Z)/12:0)", which is the 
...
DT::datatable(paths,options = list(scrollX = TRUE))
```
