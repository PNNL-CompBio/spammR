...
[1000 Soils Project Shiny App](https://shinyproxy.emsl.pnnl.gov/app/1000soils). 
...
(i.e. no `NaN` values). The sample metadata conmtains infomratiomn about the 
...
differently to what would be expected. To this end we are relying on much 
looser p-value thresholds than what normally would be considered acceptable, 
i.e. we are considering everything with an adjusted p-value of < 0.1 as 
being statistically significant for up / down regulation.
...
K00005 (glycerol dehydrogenase) being one of the "upregulated KOs" we can 
...
Next we are performing the enrichment analysis, again with the less stringent 
p-value cut-off of p-value < 0.1
```{r enrich ora}
